    ...
    public class CompleteHealthCheckRunner : IDisposable
    {
            ...
            string outputFolder)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                // First, run the script content (usually CREATE PROCEDURE)
                using (var command = new SqlCommand(scriptContent, connection))
                {
                    ...
                    //{
                    //    // Procedure already exists, continue
                    //}
                    ...
                    foreach (string batch in batches)
                    {
                        ...
                        try
                        {
                            using (SqlCommand cmd = new SqlCommand(trimmed, connection))
                            {
                        ...
                        {
                           // LogMessage(string.Format("Batch error: {0}", ex.Message));
                        }
                    }
                }
